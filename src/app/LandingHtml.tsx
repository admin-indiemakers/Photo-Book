  ...
  <>
    ...
    <nav className="nav" id="nav">
      <div className="container nav__inner">
        ...
        <div className="nav__right" style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
          {user ? (
            <>
              <span style={{ fontSize: '12px', fontWeight: '500', color: 'var(--text)', opacity: 0.8 }}>
                Hi, {user.user_metadata?.full_name?.split(' ')[0] || 'User'}
              </span>
              <button onClick={handleLogout} className="nav__login" data-magnetic style={{ background: 'transparent', border: '1px solid var(--orange)', color: 'var(--orange)' }}>
                Logout
              </button>
            </>
          ) : (
